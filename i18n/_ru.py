...
STRINGS: dict[str, str] = {
    ...
    "languages.ai.title": "ИИ-обработка текста",
    "languages.ai.hint": "Действует на перевод, исправление и переписывание — как кнопки в окне, так и действия по хоткеям.",
    "languages.ai.max_tokens.label": "Макс. длина ответа",
    "languages.ai.max_tokens.hint": "Верхний предел одного ответа ИИ в токенах (примерно 3–4 символа на токен). Увеличьте, если длинные тексты приходят обрезанными — оплачиваются только фактически сгенерированные токены.",
    ...
    "transcription.backend.hint.gemini": "точный, свой словарь терминов, есть бесплатный тариф",
    ...
    "transcription.keys.gemini.label": "Ключ Gemini",
    "transcription.keys.gemini.placeholder": "Введите ключ API Gemini",
    "transcription.keys.gemini.nudge": "← Введите ключ Gemini",
    "transcription.keys.gemini.help.title": "Как создать ключ API Gemini (бесплатно)",
    "transcription.keys.gemini.help.step1": "1. Войдите на aistudio.google.com",
    "transcription.keys.gemini.help.step2": "2. Откройте «Get API key» → «Create API key»",
    "transcription.keys.gemini.help.step3": "3. Скопируйте его и вставьте сюда",
    "transcription.keys.gemini.help.link": "Открыть Google AI Studio",
    ...
    "main.button.clear_styles": "Убрать стили",
